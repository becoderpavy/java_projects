<%@page import="com.emp.db.DBConnect"%>
<%@page import="com.emp.dao.UserDAO"%>
<%@page import="com.emp.entity.UserDtls"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<%@page import="java.sql.*"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@page isELIgnored="false"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>New User Registration</title>
<link href="css/style.css" rel="stylesheet" type="text/css" />
</head>
<body style="background-color: #eceff1;">
	<c:if test="${empty userobj}">
		<c:set value="Please Login" var="failedmsg" scope="session"></c:set>
		<c:redirect url="../index.jsp"></c:redirect>
	</c:if>
	<%@include file="../common_nav/admin_nav.jsp"%>
	<div class="container ">
		<div class="row justify-content-center">
			<div class="col-lg-8 col-md-10">
				<div class="forgot">
					<h3 class="display-4 text-center">Employee Registration</h3>
					<c:if test="${not empty succMsg }">
						<div class="alert alert-success" role="alert">${ succMsg}</div>
						<c:remove var="succMsg" />
					</c:if>

					<c:if test="${not empty failedMsg }">
						<div class="alert alert-danger" role="alert">${ failedMsg}</div>
						<c:remove var="failedMsg" />
					</c:if>
				</div>
				<%
				int id = Integer.parseInt(request.getParameter("id"));
				UserDAO dao = new UserDAO(DBConnect.getConnection());
				UserDtls u = dao.getUser(id);
				%>

				<form id="reg-form" action="../updateemp" method="POST">
					<div class="card mt-4">
						<div class="card-body">
							<div class="row">
								<div class="col">
									<label>First Name</label> <input name="fn" type="text"
										class="form-control" placeholder="First name" required
										value="<%=u.getFirstName()%>">
								</div>
								<div class="col">
									<label>First Name</label> <input name="ln" type="text"
										class="form-control" placeholder="Last name" required
										value="<%=u.getLastName()%>">
								</div>
							</div>
							<br>
							<div class="row">
								<div class="col">
									<label>Username</label> <input name="un" type="text"
										class="form-control" placeholder="User name" required
										value="<%=u.getUserName()%>">
								</div>
								<div class="col">
									<label>Password</label> <input name="ps" type="text"
										class="form-control" placeholder="Password" required
										value="<%=u.getPassword()%>">
								</div>
							</div>

							<br>
							<div class="row">

								<div class="col">
									<label>Email</label> <input name="em" type="email"
										class="form-control" placeholder="Email" required
										value="<%=u.getEmail()%>">
								</div>
								<div class="col">
									<label>Contact No</label> <input name="cn" type="number"
										class="form-control" placeholder="Contact" required
										value="<%=u.getContactNo()%>">
								</div>
							</div>

							<br>
							<div class="row">

								<div class="col">
									<label>Salary</label> <input name="sa" type="text"
										class="form-control" placeholder="Salary" required
										value="<%=u.getSalary()%>">
								</div>
								<div class="col">
									<label>Qualification</label> <input name="qu" type="text"
										class="form-control" placeholder="Qualification" required
										value="<%=u.getQualification()%>">
								</div>

								<div class="col-md-4">
									<label>Status</label> <select class="form-control" name="st">
										<option value="<%=u.getStatus()%>"><%=u.getStatus()%></option>
										<option value="Active">Active</option>
										<option value="Inactive">Inactive</option>
									</select>
								</div>

							</div>

							<div class="form-group mt-2">
								<label>Address</label>
								<textarea rows="4" cols="" class="form-control" name="ad"
									placeholder="Address"><%=u.getAdress()%></textarea>
							</div>
							<input type="hidden" value="<%=u.getId()%>" name="id">
							<button class="btn btn-info" type="submit">Submit</button>
						</div>
					</div>
					<div class="card-footer"></div>
				</form>
			</div>
		</div>
	</div>
</body>
</html>