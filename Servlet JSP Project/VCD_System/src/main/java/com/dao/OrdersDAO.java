...
public class OrdersDAO {
	...
	public boolean addOrder(List<Orders> list) {
		boolean f = false;
		try {
			String sql = "insert into orders(order_id,movie_id,user_id,quantity,total_amt,payment_type,store_id) values(?,?,?,?,?,?,?)";
			conn.setAutoCommit(false);
			PreparedStatement ps = conn.prepareStatement(sql);

			for (Orders o : list) {
				ps.setString(1, o.getOrderId());
				ps.setInt(2, o.getMovieId());
				ps.setInt(3, o.getUserId());
				ps.setInt(4, o.getQuantity());
				ps.setString(5, o.getTotalAmount());
				ps.setString(6, o.getPaymentType());
				ps.setInt(7, o.getStoreId());
	...
	public List<Orders> getOrdersByStore(int artId) {
		List<Orders> list = new ArrayList<Orders>();
		Orders order = null;
		try {
