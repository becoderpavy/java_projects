...
<body style="background-color: #f0f1f2;">
	...
	<div class="container">
		<div class="row">
			...
			String sql = "";
			if ("all".equals(ca)) {
				sql = "select * from orphans where status='Approved' order by id desc ";
			} else {
				sql = "select * from orphans where status='Approved' and category='" + ca + "'  order by id desc ";
			}
			
			OrphansDAO dao = new OrphansDAO(DBConnect.getConn());
			List<Orphans> list = dao.getAllPatientByStatus(sql); 
			//System.out.print(list);
			for (Orphans p : list) {
			%>

			<a href="orphan_view.jsp?pid=<%=p.getId()%>">
